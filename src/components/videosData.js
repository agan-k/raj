import ellstandissa from '../assets/video_thumbs/ellstandissa.webp'
import pistils from '../assets/video_thumbs/pistils.webp'
import elephant from '../assets/video_thumbs/elephant.webp'
import welikadawatte_birdland2019 from '../assets/video_thumbs/welikadawatte_birdland2019.webp'
import hirimbura from '../assets/video_thumbs/hirimbura.webp'
import welikadawatte_tijc2020 from '../assets/video_thumbs/welikadawatte_tijc2020.webp'
import galadari from '../assets/video_thumbs/galadari.webp'
import vinyl_pistils from '../assets/video_thumbs/vinyl_pistils.jpg'

const videosData = [
   {
      id: 1,
      videos_showcase: true,
      video_thumbnail: ellstandissa,
      caption: 'Ellstandissa',
      video_url: 'https://youtu.be/n1lvfw0MWMo',
   },
   {
      id: 2,
      videos_showcase: true,
      video_thumbnail: elephant,
      caption: 'The Elephant',
      video_url: 'https://youtu.be/IzkBH29oJ3M'
   },
   {
      id: 3,
      videos_showcase: true,
      video_thumbnail: pistils,
      caption: 'Pistils',
      video_url: 'https://youtu.be/vcvMQdFWsnE'
   },
   {
      id: 4,
      videos_showcase: true,
      video_thumbnail: welikadawatte_birdland2019,
      caption: 'Welikadawatte',
      video_url: 'https://youtu.be/OW3TRxUqREY'
   },
   {
      id: 5,
      videos_showcase: true,
      video_thumbnail: hirimbura,
      caption: 'Hirimbura',
      video_url: 'https://youtu.be/f0IfFxGZPlI'
   },
   {
      id: 6,
      videos_showcase: true,
      video_thumbnail: welikadawatte_tijc2020,
      caption: 'The Elephant',
      video_url: 'https://youtu.be/4JzT8WLun3g'
   },
   {
      id: 7,
      videos_showcase: true,
      video_thumbnail: galadari,
      caption: 'Galadari',
      video_url: 'https://youtu.be/Gbzoz4_TFZw'
   },
   {
      id: 8,
      videos_showcase: 'no',
      video_thumbnail: vinyl_pistils,
      caption: `'Pistils' vinyl LP pressing at Nordsø Records`,
      video_url: 'https://youtu.be/IBRLlL8KLKs'
   },

]
export default videosData

